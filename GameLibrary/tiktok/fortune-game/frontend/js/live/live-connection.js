...
class LiveConnectionManager {
    ...
    async restoreConnection() {
        try {
            const savedLiveId = localStorage.getItem('currentTikTokLiveId');
            if (savedLiveId) {
    ...
    saveConnectionState(liveId) {
        try {
            if (liveId) {
                localStorage.setItem('currentTikTokLiveId', liveId);
            }
        } catch (error) {
    ...
    clearConnectionState() {
        try {
            localStorage.removeItem('currentTikTokLiveId');
        } catch (error) {
    ...
    async connect() {
        ...
        if (!liveId) {
            this.updateStatus('请输入TikTok用户名', 'error');
            return;
        }

        try {
            this.updateStatus('正在连接TikTok...', '');
    ...
    async disconnect() {
        ...
        try {
            const response = await fetch('http://localhost:5001/api/live/stop', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
