"""
巫女占卜游戏 TikTok版 - 后端API
提供占卜聊天、TikTok直播集成、AI Agent等完整功能
支持按需启动和用户级服务隔离
"""
...
try:
    ...
    print('[Fortune-Game-TikTok] 会话管理器已加载')
except ImportError as e:
    SESSION_MANAGER_AVAILABLE = False
    print(f'[Fortune-Game-TikTok] 会话管理器不可用: {e}')
...
try:
    ...
    AI_AVAILABLE = True
    print('[Fortune-Game-TikTok] AI Agent服务类已加载')
except ImportError as e:
    AI_AVAILABLE = False
    FortuneAgentLLMX = None
    LiveService = None
    print(f'[Fortune-Game-TikTok] AI Agent服务类不可用: {e}')

try:
    from services.tiktok.live_monitor import LiveRoomMonitor
    LIVE_AVAILABLE = True
    print('[Fortune-Game-TikTok] TikTok直播服务类已加载')
except ImportError as e:
    LIVE_AVAILABLE = False
    LiveRoomMonitor = None
    print(f'[Fortune-Game-TikTok] TikTok直播服务类不可用: {e}')

# 创建游戏蓝图 - 使用TikTok专用路径
fortune_bp = Blueprint('fortune_game_tiktok', __name__, url_prefix='/api/fortune-tiktok')
...
def get_or_create_user_session(user_id: str):
    ...
    try:
        session = game_session_manager.create_session(user_id, 'fortune-game-tiktok')
        return session
    except Exception as e:
        print(f'[Fortune-Game-TikTok] 创建会话失败: {e}')
        return None
...
def get_user_fortune_agent(user_id: str):
    ...
    try:
        ...
        print(f'[Fortune-Game-TikTok] 为用户 {user_id} 创建AI Agent')
        return agent
    except Exception as e:
        print(f'[Fortune-Game-TikTok] 创建AI Agent失败: {e}')
        return None
...
def get_user_live_service(user_id: str):
    ...
    try:
        ...
        print(f'[Fortune-Game-TikTok] 为用户 {user_id} 创建TikTok直播服务')
        return service
    except Exception as e:
        print(f'[Fortune-Game-TikTok] 创建TikTok直播服务失败: {e}')
        return None
...
def start_live():
    ...
    try:
        ...
        if LIVE_AVAILABLE and LiveRoomMonitor:
            try:
        ...
        if LIVE_AVAILABLE:
            try:
                ...
                if SESSION_MANAGER_AVAILABLE and game_session_manager:
                    session = game_session_manager.get_user_session(user_id, 'fortune-game-tiktok')
                    if session:
                        # 只清理直播相关服务，保留其他服务
...
def close_session():
    ...
    try:
        ...
        if SESSION_MANAGER_AVAILABLE and game_session_manager:
            game_session_manager.close_user_session(user_id, 'fortune-game-tiktok')
            return jsonify({
                'success': True,
                'message': '会话已关闭',
                'user_id': user_id
