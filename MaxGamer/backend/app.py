...
# 项目根目录（MaxGamer 和 GameLibrary 的父目录）
...
app.config['SECRET_KEY'] = 'maxgamer-secret-key-2024'
...
def verify_game_license(game_id: str):
    """
    验证用户的游戏授权
    从请求中获取 ticket（JWT），验证用户身份并检查游戏授权

    返回: (success: bool, user_id: int | None, error_message: str | None, days_remaining: int | None)
    """
    from database import get_db_session, License
    ...
    if not token:
        return False, None, '未提供认证令牌', None

    # 验证 JWT
    payload = verify_access_token(token)
    if not payload:
        return False, None, '认证令牌无效或已过期', None

    user_id = payload.get('user_id')
    if not user_id:
        return False, None, '令牌中缺少用户信息', None
    ...
    try:
        ...
        if not license_record:
            return False, user_id, '您尚未购买此游戏的授权', None
        ...
        if license_record.expires_at:
            if license_record.expires_at < datetime.utcnow():
                ...
                db.commit()
                return False, user_id, '游戏授权已过期，请续费', None

            # 计算剩余天数
            delta = license_record.expires_at - datetime.utcnow()
            days_remaining = max(0, delta.days)
        else:
            days_remaining = None  # 永久授权

        return True, user_id, None, days_remaining
    finally:
        db.close()
...
def start_live():
    ...
    try:
        # 验证游戏授权
        success, user_id, error_msg, days_remaining = verify_game_license('fortune-game')
        if not success:
            return jsonify({
                'success': False,
                'message': error_msg,
        ...
        if LIVE_SERVICE_AVAILABLE and live_service:
            ...
            if result.get('success'):
                result['license'] = {
                    ...
                    'is_permanent': days_remaining is None
                }
...
if __name__ == '__main__':
    ...
    print(f'[OK] MaxGamer Python 后端启动在端口 {PORT}')
