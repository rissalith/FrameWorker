...
const MyApps = {
    ...
    async init() {
        ...
        try {
            ...
            if (!AuthManager.currentUser) {
                console.log('[MyApps] 等待用户信息加载...');
                try {
                    await AuthManager.getUserInfo();
                } catch (e) {
                    console.warn('[MyApps] 获取用户信息失败，继续加载:', e.message);
                }
            }

            await this.loadLicenses();
        } catch (error) {
            console.error('[MyApps] 初始化失败:', error);
            const grid = document.getElementById('appsGrid');
            if (grid) {
                grid.innerHTML = `
                    <div class="error-state" style="text-align: center; padding: 40px; grid-column: 1/-1;">
                        <p style="color: #d93025; margin-bottom: 16px;">初始化失败: ${error.message}</p>
                        <button class="btn-primary" onclick="MyApps.init()">重试</button>
                    </div>
                `;
            }
        }
    ...
    async loadLicenses() {
        ...
        console.log('[MyApps] API地址:', AuthManager.apiBaseUrl);
        ...
        try {
            const url = `${AuthManager.apiBaseUrl}/products/my-licenses`;
            console.log('[MyApps] 请求URL:', url);
            console.log('[MyApps] Token:', AuthManager.token ? AuthManager.token.substring(0, 20) + '...' : '无');
            ...
            console.log('[MyApps] API 返回数据:', data);
            ...
            if (grid) {
                grid.innerHTML = `
                    <div class="error-state" style="text-align: center; padding: 40px; grid-column: 1/-1;">
                        <p style="color: #d93025; margin-bottom: 16px;">加载失败: ${errorMsg}</p>
                        <p style="color: #666; font-size: 12px; margin-bottom: 16px;">API: ${AuthManager.apiBaseUrl}/products/my-licenses</p>
     ...
     */
    renderLicenses() {
        console.log('[MyApps] renderLicenses 开始, licenses数量:', this.licenses.length);

        const grid = document.getElementById('appsGrid');
        const emptyState = document.getElementById('emptyState');
        const countEl = document.getElementById('appCount');

        console.log('[MyApps] DOM元素状态:', {
            grid: grid ? '存在' : '不存在',
            emptyState: emptyState ? '存在' : '不存在',
            countEl: countEl ? '存在' : '不存在'
        });

        // 合并 fortune-game 和 fortune-game-tiktok 的授权
        // 它们现在是同一个游戏（巫女上上签），只是支持不同平台
        const mergedLicenses = this.mergeFortuneLicenses(this.licenses);

        // 更新计数
        if (countEl) {
            countEl.textContent = `${mergedLicenses.length} 个应用`;
        }

        if (mergedLicenses.length === 0) {
            console.log('[MyApps] 无应用，显示空状态');
            ...
            if (emptyState) {
                ...
                emptyState.innerHTML = `
                    ...
                    <h3>暂无应用</h3>
                    <p>您还没有购买任何游戏，去商城看看吧</p>
                    <button class="btn-primary" onclick="Router.navigate('game-market')">
                        浏览游戏市场
                    </button>
                `;
            }
        ...
        console.log('[MyApps] 有应用，渲染卡片');
        if (grid) grid.style.display = 'grid';
        if (emptyState) emptyState.style.display = 'none';

        if (grid) {
            const cardsHtml = mergedLicenses.map(license => this.renderAppCard(license)).join('');
            console.log('[MyApps] 渲染的卡片数量:', mergedLicenses.length);
            grid.innerHTML = cardsHtml;
        }
    },

    /**
     * 合并占卜游戏授权（fortune-game 和 fortune-game-tiktok 合并为一个）
     */
    mergeFortuneLicenses(licenses) {
        const fortuneGameIds = ['fortune-game', 'fortune-game-tiktok'];
        const fortuneLicenses = licenses.filter(l => fortuneGameIds.includes(l.game_id));
        const otherLicenses = licenses.filter(l => !fortuneGameIds.includes(l.game_id));

        if (fortuneLicenses.length === 0) {
            return otherLicenses;
        }

        // 找出最优的授权（有效期最长的，或永久的）
        let bestLicense = fortuneLicenses[0];
        for (const license of fortuneLicenses) {
            // 优先选择状态为 active 的
            if (license.status === 'active' && bestLicense.status !== 'active') {
                bestLicense = license;
                continue;
            }
            // 永久授权优先
            if (!license.expires_at && bestLicense.expires_at) {
                bestLicense = license;
                continue;
            }
            // 剩余天数更长的优先
            if (license.days_remaining > (bestLicense.days_remaining || 0)) {
                bestLicense = license;
            }
        }
        ...
        const mergedLicense = {
            ...bestLicense,
            game_id: 'fortune-game',  // 统一使用 fortune-game
            // 保存所有相关授权的 ID，用于后续操作
            _all_license_ids: fortuneLicenses.map(l => l.id),
            _platforms: fortuneLicenses.map(l => l.game_id.includes('tiktok') ? 'tiktok' : 'douyin')
        };

        return [mergedLicense, ...otherLicenses];
    },
    ...
    renderAppCard(license) {
        ...
        const gameIcons = {
            ...
            'fortune-game-tiktok': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                <circle cx="12" cy="12" r="10"/>
                <path d="M12 6v2M12 16v2M6 12h2M16 12h2"/>
                <circle cx="12" cy="12" r="3"/>
            </svg>`,
        ...
        const getGameName = (gameId) => {
            if (gameId === 'fortune-game' || gameId === 'fortune-game-tiktok') {
                return window.I18n ? I18n.t('game_miko_fortune') : '巫女上上签';
            }
            return gameId;
        };

        const iconSvg = gameIcons[license.game_id] || gameIcons.default;
        const name = getGameName(license.game_id);
            ...
            <div class="app-card" data-license-id="${license.id}">
                ...
                <div class="app-card-footer">
                    ...
                    ` : `
                        ...
                        <button class="btn-app danger" onclick="MyApps.deleteLicense('${license.id}', '${name}')">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px;">
     ...
     * 删除授权
     */
    async deleteLicense(licenseId, gameName) {
        ...
        try {
            const response = await AuthManager.authenticatedFetch(
                `${AuthManager.apiBaseUrl}/products/license/${licenseId}`,
                {
                    method: 'DELETE'
                }
            );
...
// 添加 toast 动画样式（使用 IIFE 避免变量冲突）
(function() {
    const myAppsStyle = document.createElement('style');
    myAppsStyle.textContent = `
        @keyframes toastIn {
    ...
    `;
    document.head.appendChild(myAppsStyle);
})();
