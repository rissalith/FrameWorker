...
class LiveConnectionManager {
    ...
    async connect() {
        ...
        try {
            ...
            if (!this.socket) {
                ...
                // Socket.IO 连接事件 - 仅用于调试，不代表直播间连接状态
                this.socket.on('connect', () => {
                    // 静默处理，避免误导用户
                });
                
                this.socket.on('disconnect', () => {
                    console.warn('⚠️ Socket.IO 连接已断开，可能影响直播消息接收');
                });
            ...
            // 发送开始监听请求
            const response = await fetch('http://localhost:3000/api/fortune/live/start', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                ...
                // 输出清晰的成功日志
                console.log(`✅ 直播间连接成功: ${liveId}`);
                console.log(`📡 后端返回: ${result.message}`);
                ...
                console.error(`❌ 直播间连接失败: ${result.message}`);
                this.isRestoring = false;
            }
        } catch (error) {
    ...
    async disconnect() {
        ...
        try {
            const response = await fetch('http://localhost:3000/api/fortune/live/stop', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
