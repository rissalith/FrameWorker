...
echo "备份数据库..."
if [ -f "$PROJECT_DIR/XMGamer/backend/frameworker.db" ]; then
    cp $PROJECT_DIR/XMGamer/backend/frameworker.db $BACKUP_DIR/frameworker_$TIMESTAMP.db
fi

echo "备份 .env 文件..."
if [ -f "$PROJECT_DIR/XMGamer/backend/.env" ]; then
    cp $PROJECT_DIR/XMGamer/backend/.env $BACKUP_DIR/.env_$TIMESTAMP
fi
...
if [ -d ".git" ]; then
    ...
    echo "2. 手动上传文件: scp -r XMGamer/frontend/* root@149.88.69.87:/var/www/FrameWorker/XMGamer/frontend/"
    echo ""
...
cd $PROJECT_DIR/XMGamer/backend
...
# 更新静态文件的修改时间，强制浏览器刷新
echo "更新静态文件时间戳..."
find $PROJECT_DIR/XMGamer/frontend -type f \( -name "*.css" -o -name "*.js" -o -name "*.html" \) -exec touch {} \;
...
if command -v docker &> /dev/null; then
    ...
    if docker ps | grep -q xmgamer-gateway; then
        docker restart xmgamer-gateway
        echo "✅ xmgamer-gateway 已重启"
    else
        echo "⚠️  xmgamer-gateway 容器未运行"
    fi
    
    if docker ps | grep -q xmgamer-api; then
        docker restart xmgamer-api
        echo "✅ xmgamer-api 已重启"
    else
        echo "⚠️  xmgamer-api 容器未运行"
    fi
fi
